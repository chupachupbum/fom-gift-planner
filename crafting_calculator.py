...
Recursive Crafting Calculator and DAG Resolver for Fields of Mistria.
Determines item availability (HAVE=2, CRAFT=1, UNAVAILABLE=0), calculates
maximum craftable quantities, resolves multi-step intermediate crafting chains
with shared raw material pool simulation, and generates human-readable crafting steps.
"""
...
def load_recipes(recipes_path: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if recipes_path is None:
        root_path = Path(__file__).resolve().parent / "data" / "recipes.json"
        parent_path = Path(__file__).resolve().parent.parent / "data" / "recipes.json"
        if root_path.exists():
            target = root_path
